  ...
  <div v-bind="resolvedAttrs.wrapperAttrs.wrapper1">
    ...
    <div v-bind="resolvedAttrs.wrapperAttrs.wrapper2">
      <label class="relative inline-block w-8 h-4">
        ...
        />
        <span
          class="absolute cursor-pointer top-0 left-0 right-0 bottom-0 bg-gray-400 dark:bg-gray-600 transition-all duration-100 ease-in-out rounded-[0.75rem] peer-checked:bg-gray-900 dark:peer-checked:bg-dark-text"
        ></span>
        <span
          class="absolute w-3 h-3 top-1/2 left-[0.125rem] bg-white dark:bg-dark-surface transition-all duration-100 ease-in-out rounded-full shadow-toggle dark:shadow-dark-toggle transform -translate-y-1/2 peer-checked:translate-x-[1rem]"
        ></span>
      </label>
      <label v-bind="resolvedAttrs.labelAttrs">{{ label }} </label>
    </div>
  </div>
</template>
...
const inputConfig = {
  ...
  elm: {
    addClass:
      props.version === "dashboard" ? "opacity-0 w-0 h-0 peer" : props.version === "auth" ? "" : "",
    addAttributes: {
      type: props.version,
    },
  },
  additionalConfig: {
    label: {
      addClass:
        props.version === "dashboard"
          ? "text-base font-medium text-gray-900 dark:text-dark-text cursor-pointer"
          : props.version === "auth"
