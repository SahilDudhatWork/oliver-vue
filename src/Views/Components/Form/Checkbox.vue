<template>
  <div v-bind="resolvedAttrs.wrapperAttrs.wrapper1">
    ...
    <label v-if="label" :for="id" v-bind="resolvedAttrs.labelAttrs">
      <span class="text-gray-900 dark:text-dark-text leading-5 text-sm text-wrap">{{ label }}</span>
      ...
      <div v-if="tags && tags.length > 0">
        ...
        >
          <div
            class="text-gray-900 dark:text-dark-text text-right font-medium text-xs leading-[1.125rem] inline-flex px-1.5 justify-center items-center gap-2.5 rounded-tag bg-published dark:bg-dark-published"
          >
            {{ tag.text }}
          </div>
        </span>
...
const inputConfig = {
  ...
  elm: {
    addClass:
      props.version === "dashboard"
        ? 'appearance-none bg-white dark:bg-dark-surface border border-gray-300 dark:border-dark-border rounded-input w-4 min-w-4 h-4 mr-2 ml-[0.188rem] checked:accent-success checked:bg-success dark:checked:bg-dark-success checked:border-success dark:checked:border-dark-success checked:relative checked:after:content-[""] checked:after:absolute checked:after:left-[0.3rem] checked:after:top-[0.15rem] checked:after:w-1 checked:after:h-2 checked:after:border checked:after:border-solid checked:after:border-t-0 checked:after:border-l-0 checked:after:border-black dark:checked:after:border-white checked:after:border-w-0 checked:after:border-b-2 checked:after:border-r-2 checked:after:rotate-45 checked:after:box-border cursor-pointer'
        : props.version === "auth"
        ? ""
        : "",
    addAttributes: {
  ...
  additionalConfig: {
    label: {
      addClass:
        props.version === "dashboard"
          ? "flex items-center gap-2 text-gray-900 dark:text-dark-text leading-5 text-sm whitespace-nowrap overflow-hidden text-ellipsis cursor-pointer"
          : props.version === "auth"
